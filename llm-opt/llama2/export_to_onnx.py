...
def get_model_inputs(config: LlamaConfig, device):
    ...
    input_ids = torch.randint(low=0, high=config.vocab_size, size=(batch_size, seq_len), dtype=torch.int64, device=device)
...
def run_dynamo_export(args: argparse.Namespace, l_config: LlamaConfig, llama: LlamaForCausalLM, rank=0, world_size=1):
    ...
    output_path = os.path.join(args.output_name, f"{args.model_name}_decoder_with_past_model_fp32.onnx")
    onnx_model = onnx.load_model(temp_path, load_external_data=True)
...
def run_torchscript_export(args: argparse.Namespace, l_config: LlamaConfig, llama: LlamaForCausalLM, rank=0, world_size=1):
    # Export decoder_model.onnx
    ...
    #output_path = os.path.join(args.output_name, f"{args.model_name}_decoder_model_fp32.onnx")
    output_path = f"{args.output_name}_rank-{rank}_decoder_model_fp32.onnx"
    onnx_model = onnx.load_model(temp_path, load_external_data=True)
    save_onnx_model(
        onnx_model,
        output_path,
